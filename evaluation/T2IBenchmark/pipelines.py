...
def collect_images_recursive(
    ...
    """
    Recursively collect image file paths under root_dir.
    Returns a sorted list of absolute paths (stable order).
    """
    exts = exts or IMAGE_EXTS_DEFAULT
    root = Path(root_dir)
    if not root.exists():
        raise FileNotFoundError(f"Directory not found: {root_dir}")

    paths: List[str] = []
    # rglob follows symlinks only if we manually allow it via os.walk; Path.rglob won't.
    # We'll use os.walk for explicit followlinks support.
...
# -------------------------
# 3) Collectors for your folder structure
#    ./results/{method}/{category}/{concept}/{index}.png
# -------------------------
def collect_images_for_method(results_root: str, method: str) -> List[str]:
    method_dir = os.path.join(results_root, method)
    return collect_images_recursive(method_dir)

def collect_images_for_category(results_root: str, method: str, category: str) -> List[str]:
    cat_dir = os.path.join(results_root, method, category)
    return collect_images_recursive(cat_dir)


# -------------------------
# 4) Wrapper functions to compute FID with your existing calculate_fid
# -------------------------
def calculate_fid_for_methods(
    results_root: str,
    method_a: str,
    method_b: Union[str, List[str], BaseImageLoader, FIDStats],
    *,
    device="cuda",
    seed: int = 42,
    batch_size: int = 128,
    ...
    """
    FID between all images under:
      ./results/{method_a}/**/*
      ./results/{method_b}/**/*
    """
    paths_a = collect_images_for_method(results_root, method_a)
    paths_b = create_dataset_from_input(method_b)

    if verbose:
        print(f"[Method] {method_a}: {len(paths_a)} images")


    # call your existing function
    fid, (info_a, info_b) = calculate_fid(
        paths_a, paths_b,
        device=device,
        seed=seed,
        batch_size=batch_size,
    ...
    return fid, (info_a, info_b)


def calculate_fid_for_categories(
    results_root: str,
    method_a: str,
    category: str,
    method_b: Union[str, List[str], BaseImageLoader, FIDStats],
    *,
    ...
    """
    FID between all images under:
      ./results/{method_a}/{category}/**/*
      ./results/{method_b}/{category}/**/*
    """
    paths_a = collect_images_for_category(results_root, method_a, category)
    paths_b = create_dataset_from_input(method_b)

    if verbose:
        print(f"[Category={category}] {method_a}: {len(paths_a)} images")
