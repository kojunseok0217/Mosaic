...
#         value = value.repeat_interleave(query.size(-3)//value.size(-3), -3)
    ...
    # Return the post-softmax attention weights for visualization/attribution.
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        # `sample` projections.
        def call_proj(module, x, text_mask):
            if hasattr(module, "uses_text_mask"):
                return module(x, text_mask=text_mask)
            else:
                return module(x)
        query = call_proj(attn.to_q, hidden_states, text_mask)
        key   = call_proj(attn.to_k, hidden_states, text_mask)
        value = call_proj(attn.to_v, hidden_states, text_mask)
