...
# Lora for Single Stream Transformer
from typing import Any, Optional, Union

class MaskLoraLinear(Linear):
    """
    PEFT-compatible LoRA Linear with optional token-wise text_mask gating.

    - text_mask is optional.
    - If text_mask is None: behaves like standard PEFT LoRA Linear.
    - If text_mask is provided: applies LoRA delta only where mask == 1.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.uses_text_mask = True

    def forward(self, x: torch.Tensor, *args: Any, text_mask=None,  **kwargs: Any) -> torch.Tensor:
        self._check_forward_args(x, *args, **kwargs)
        adapter_names = kwargs.pop("adapter_names", None)
        ...
        if self.disable_adapters:
            ...
            # mixed-batch adapter inference path
            # text_mask와 adapter_names를 동시에 섞는 건 일단 지원하지 않는 편이 안전
            if text_mask is not None:
                raise ValueError("text_mask is not supported together with adapter_names in MaskLoraLinear.")
            result = self._mixed_batch_forward(x, *args, adapter_names=adapter_names, **kwargs)
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    delta = self.lora_magnitude_vector[active_adapter](
                        x_drop,
                        lora_A=lora_A,
                        lora_B=lora_B,
                        scaling=scaling,
                ...
                if text_mask is not None:
                    ...
                    delta = delta * text_mask.to(delta.dtype)
