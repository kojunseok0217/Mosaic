...
from tools.ir_concept import UniversalModelCaller, MoE, MoE_local_qwen
...
def finetune(args):
    ...
    #     wandb.init(
    #         project=getattr(args, "wandb_project", "flux-lora"),
    #         name=getattr(args, "wandb_run_name", Path(args.output_dir).name),
    #         config=vars(args),
    #     )
    ...
    # Make irrelevant concepts first

    # irrelevant_concepts = MoE(model_caller, args.key_word, 3, model_list=["qwen"])
    irrelevant_concepts = MoE_local_qwen(args.key_word, 3, devices[0], model_list=["qwen"])
    print(irrelevant_concepts)
    # irrelevant_concepts = ['ocean', 'weather', 'wing']

    print("irrelevant concepts : ", irrelevant_concepts)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            save_path = os.path.join(args.output_dir, f"checkpoint-{args.key_word}-{global_step}") # 어떤 컨셉을 지운 모델인지 이름에 넣기
